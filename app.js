(function () {
  ...
  function getTodayStatus(company) {
    const hasClosedDay = company.closedMonday || company.closedFriday;
    if (!hasClosedDay) {
      return {
        color: "#ffcc00",
        closedToday: false,
        label: "Fermeture non renseignee"
      };
    }

    const today = new Date().getDay();
    const closedToday = (today === 1 && company.closedMonday) || (today === 5 && company.closedFriday);

    return {
      color: closedToday ? "#d40511" : "#15803d",
      closedToday,
      label: closedToday ? "Ferme aujourd'hui" : "Ouvert aujourd'hui"
    };
  }
